...
/** @brief Initialize the power supply, screen and other peripherals
  */
void M5EPD::begin(bool touchEnable, bool SDEnable, bool BatteryADCEnable, bool I2CEnable)
{
    ESP_LOGI(TAG, "M5EPD::begin.");
    ...
    if (SDEnable == true)
    {
        SPI.begin(M5EPD_MOSI_PIN, M5EPD_MISO_PIN, M5EPD_MOSI_PIN, M5EPD_SDCARD_CS_PIN);
        SD.begin(M5EPD_SDCARD_CS_PIN, SPI, 20000000);
    }
    ...
    if (touchEnable == true)
    {
        ...
        {
            ESP_LOGE(TAG, "Touch pad initialization failed.");
        }
    }
