...
void EPDGUI_Button::setBMPButton(String label_l, String label_r,
    ...
    _CanvasNormal->setFreeFont(CR12);
    _CanvasNormal->setTextSize(1);
    _CanvasNormal->setTextColor(15);
    if (label_l.length()) {
        _CanvasNormal->setTextDatum(CL_DATUM);
        _CanvasNormal->drawString(label_l, 47 + 8, (_h >> 1) + 5 - 7);
    }
    if (label_r.length()) {
        _CanvasNormal->setTextDatum(CR_DATUM);
        _CanvasNormal->drawString(label_r, _w - 15, (_h >> 1) + 5 - 7);
    }
