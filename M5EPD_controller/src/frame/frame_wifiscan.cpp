...
Frame_WifiScan::Frame_WifiScan(void) : Frame_Base(true) {
    ...
    for (int i = 0; i < MAX_BTN_NUM; i++) {
        ...
        _key_wifi[i]->CanvasNormal()->setTextSize(2);
    ...
    if (_language == LANGUAGE_FR) {
        exitbtn("Menu");
        _canvas_title->drawString("Reseau", 270, 34-10);
    }
    else {
        exitbtn("Menu");
        _canvas_title->drawString("Wlan", 270, 34-10);
    }
...
void Frame_WifiScan::DrawItem(EPDGUI_Button *btn, String ssid, int rssi) {
    ...
    btn->CanvasNormal()->setFreeFont(CR12);
    btn->CanvasNormal()->setTextSize(1);
...
void Frame_WifiScan::Connect() {
    ...
    while (WiFi.status() != WL_CONNECTED) {
        ...
        if (millis() - start_time > 10000) {
            ...
            if (_language == LANGUAGE_FR) {
                err.drawString("Mauvais mot de passe", 150, 55);
            } 
            else {
